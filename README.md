...
- Buffer - это блок памяти для записи\чтения
- Channel - в отличие от стримов может и читать и записывать файл
  - flip() - кувырок. меняет позицию для записи в чтение
  - clear() - меняет позицию на нулевой элемент и теперь идет перезапись буфера
  - понятие limit помогает установить ячейку окончания чтения
    - максимально равна capacity
  - hasRemaining имеет что удерживать)
### another methods buffer and channels
...
# REGular EXpressions (REGEX)
   - зачем отдельный класс если String поддерживает регулярки? - Производительность
   - [B-Fd-j3-8] 3 диапазона в одном выражении
   - [^0-9] - отрицание + диапазон
   - ^ начало строки
   - $ конец строки
   - . любой
...
## matches & split
   ...
   - .group()
     ...
     - DT specifiers
       ...
       - "f" float
# Enum
 - ограничение конкретным списком
 - конструктор имеет access modifier private и не нуждается во внешнем вызове
 - дочерний класс java.lang.Enum
 - часто используется 
   - valueOf()
   - values()
# Scanner
...
# Reflection
- очень полезный механизм например для задачи
  - file.txt
    - sum
    - 11
    - 21
  - попробуй вызвать метод на классе 
    - название которого лежит в текстовом файле!
    - и параметры тоже там взять надо

# Annotation - очень редко создаем?(
- @Override - спасет от:
  - ошибки в имени == не произойдет создание нового метода
  - не даст добавить параметры в метод вызова == не произойдет создание нового метода
  - не даст не заметить потерю метода у родителя == не произойдет создание нового метода
- @Deprecated - устарел
- это специальные метки\комментарии\метаданные для передачи информации.
- @Target показывает область кода, к которой Аннотация может быт ьприменима
  - TYPE = class, interface, enum.
  - FIELD = поле класса.
  - METHOD =  метод класса.
  - PARAMETER - параметры метода или конструктора.
- @Retention - жизненный цикл аннотации
  - SOURCE = аннотация видна в исходном коде, обрабат. компилятором. ЕЕ НЕТ в байт коде.
  - CLASS = видна в byte коде, НО отбрасывается JVM во время выполнения.
